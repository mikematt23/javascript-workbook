...
const stringsToNumbs = strNums.map(function(number){
     return parseInt(number)
})
console.log(stringsToNumbs)

// With the same numbers, find the sum of the even values
 const sumEvens = stringsToNumbs.filter(function(even){
  return even % 2 === 0;
 });
...
const weatherStates = weather.map(function(weatherName){
  let name =  weatherName.weather_state_name
  return name
})
...
let rightId = weather.find(function(x){
  if(x.min_temp === 15.915){
    return x
  }
})
